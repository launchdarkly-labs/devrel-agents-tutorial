...
def test_search_comparison():
    ...
    # Mock vector search tool to avoid requiring embeddings
    with patch('tools_impl.search_v2.VectorStore') as mock_vector_store:
        mock_store = Mock()
        mock_store.exists.return_value = True
        mock_store.search.return_value = [("Sample document content", 0.85, {"source": "test"})]
