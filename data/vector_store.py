...
class VectorStore:
    ...
    def get_store_info(self) -> dict:
        """Get information about the current vector store for migration guidance."""
        index_path = self.store_path / "faiss.index"
        docs_path = self.store_path / "documents.pkl"
        ...
        try:
            with open(docs_path, "rb") as f:
                ...
                return {
                    "exists": True,
                    "provider": data.get("provider", "openai"),  # Default for legacy stores
                    "model": data.get("model", "text-embedding-3-small"),
                    "dimension": data.get("dimension", 1536),
                    "documents": len(data.get("documents", []))
                }
        except Exception as e:
            log_debug(f"STORE INFO: Error reading store metadata: {e}")
            return {
                "exists": True,
                "provider": "unknown",
                "model": "unknown",
    ...
    def check_migration_compatibility(self, target_provider: str = None) -> dict:
        """
        Check compatibility for migrating to a target provider.

        Args:
            target_provider: Target provider to migrate to (default: current provider)

        Returns:
            dict: Migration compatibility information
        """
        target_provider = target_provider or self.provider
        ...
        if not store_info["exists"]:
            return {
                "compatible": True,
                "action_needed": "none",
                "message": f"No existing vector store found. New store will use {target_provider} provider.",
                "store_info": store_info
            }

        current_provider = store_info["provider"]
        current_dimension = store_info["dimension"]

        # Determine target dimensions for comparison
        if target_provider == "openai":
            target_dimension = 1536  # text-embedding-3-small
        elif target_provider == "bedrock":
            target_dimension = int(os.getenv('BEDROCK_EMBEDDING_DIMENSIONS', '1024'))
        else:
            target_dimension = "unknown"

        # Check compatibility
        if current_provider == target_provider and current_dimension == target_dimension:
            return {
                "compatible": True,
                "action_needed": "none",
                "message": f"✅ Vector store is already compatible with {target_provider} provider.",
                "store_info": store_info
            }
        elif current_provider != target_provider:
            return {
                "compatible": False,
                "action_needed": "rebuild",
                "message": f"⚠️  Provider change required: {current_provider} → {target_provider}. Vector store rebuild needed.",
                "store_info": store_info,
            ...
            return {
                "compatible": False,
                "action_needed": "rebuild",
                "message": f"⚠️  Dimension change required: {current_dimension} → {target_dimension}. Vector store rebuild needed.",
                "store_info": store_info,
                "target_provider": target_provider,
                "target_dimension": target_dimension
            }
            ...
            return {
                "compatible": True,
                "action_needed": "none",
                "message": f"✅ Vector store is compatible with {target_provider} provider.",
                "store_info": store_info
            }